...
def apply_smart_zoom(project: JyProject, video_segment, events_json_path: str, zoom_scale=150, zoom_duration_us=500000):
    ...
    if not os.path.exists(events_json_path):
        ...
        print(f"[ERROR] Events file not found: {events_json_path}")
        return
    ...
    if not click_events:
        # 注意：Windows 默认控制台编码可能为 GBK，避免输出 emoji 导致编码异常
        print("[INFO] No click events found in JSON.")
        return

    # 注意：Windows 默认控制台编码可能为 GBK，避免输出 emoji 导致编码异常
    print(f"[INFO] Found {len(click_events)} click events. Applying smart zoom keyframes...")
    ...
    print(f"[INFO] Grouped into {len(grouped_events)} zoom sessions.")
    ...
    # 注意：Windows 默认控制台编码可能为 GBK，避免输出 emoji 导致编码异常
    print("[OK] Smart zoom keyframes applied successfully.")
